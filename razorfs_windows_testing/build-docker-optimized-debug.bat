@echo off
echo Building optimized RazorFS Docker image with debug output...
echo.

REM Build the Docker image from the repository root
echo Building from repository root...
docker build --no-cache -f razorfs_windows_testing/Dockerfile.optimized.debug -t razorfs-optimized-debug .
...
echo docker run --rm -v "%cd%\results:/results" razorfs-optimized-debug
echo.
pause